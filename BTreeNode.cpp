...
namespace _nmsp1 {
    ...
    public:
        ...
        void InsertElem(const T &e)  //不可以指定插入位置，程序内部会自动确定插入位置
        {
            ...
            while (currnode != nullptr) {
                ...
                while (i < currnode->size) {
                    if (currnode->data[i] == e)
                        return; //要插入的数据与当前B树中某节点数据相同，则不允许插入，直接返回
            ...
            //该阶段包含的数据数量是否在允许的范围内，比如一个4阶B树只能有3个数据，如果超过3个数据达到了4个，则必须要拆分了
            if (currnode->size < M)
            ...
            while (true) {
                ...
                //拆分肯定会导致出现2个新的节点
                //对于4阶B树，假如有5、6、11、12四个数据，拆分的话 一个新节点放5， 另一个新节点放 11、12
                BTreeNode<T, M> *tmp1 = new BTreeNode<T, M>;
                BTreeNode<T, M> *tmp2 = new BTreeNode<T, M>;

                //给tmp1、tmp2值
                int index = 0;
                for (int k = 0; k < mididx; ++k)   //把5放入第一个新节点
                {
                ...
                //curnode可能是有孩子的，所以拆开成tmp1,tmp2后，currnode的孩子也就成了tmp1和tmp2的孩子
                index = 0;
                ...
                index = 0;
                for (int k = (mididx + 1); k <= currnode->size; ++k) //节点的指针数量要比数据元素数量多1，所以这里是k<=...
                {
                ...
                //节点的第1个第2个指针分别指向他的两个孩子，第3个指针开始全部置空
                for (int k = 2; k <= M; ++k) {
                    //该节点的其他孩子指针置空
                ...
                if (currnode->parent != nullptr) {
                    ...
                    for (int k2 = (par->size - 1); k2 >= k; --k2) //后边的数据如果有则向后移动
                    {
        ...
        void DeleteElem(const T &e) {
            ...
            iMinData -= 1;   //根节点至少可以只有1个数据，其他节点至少有⌈m/2⌉-1个数据（键）。
            int iMaxData = M - 1;//每个节点至多有m-1个数据

            //判断所删除的数据是否位于终端节点
            if (currnode->childs[0] != nullptr) //判断下标0就足够了
            {
            ...
            //非根节点
            if (currnode->size >= iMinData) //删除数据后节点中的数据数量不低于下限，不用处理，直接返回
                return;

            //删除数据后导致节点中数据数量过少，需要进行节点合并了
            while (true) {
                //如果兄弟节点数据的个数是超过数量2的，就可以从兄弟中借一个数据过来。借的方法采用“父子换位法”,优先考察紧挨着的左侧兄弟节点，若不行再考察紧挨着的右侧兄弟节点
                ...
                if (borrowidx != -1) {
                    ...
                    if (borrowidx < childidx) {
                        ...
                        //currnode和bronode的孩子指针可能都需要更新，尤其是B树高度超过2层，达到3层甚至更高，需要从下向上反复处理多个节点时
                ...
                //被删除了数据的节点就会和兄弟节点、父节点进行合并（这其实就是插入数据的逆向操作），下面的写法不唯一，大家如果有更好的写法当然可以采用
                BTreeNode<T, M> *tmpparent = currnode->parent;
                if (childidx == 0) {
                    //只能和右孩子合并，为了编码方便，数据向currnode(左孩子）集中
                    currnode->data[currnode->size] = tmpparent->data[0];//父中数据放入左孩子，父中数据肯定比左孩子都大，因此放在左孩子节点其他数据的最右侧
                    currnode->size++;

                    BTreeNode<T, M> *rightchild = tmpparent->childs[1];
                    for (int i = 0; i <
                                    rightchild->size; ++i) //rightchild数据放入左孩子,rightchild数据位于父的右侧，因此肯定比父中数据更大，所以依次放在左孩子节点其他数据最右侧
                    {
                    ...
                    leftchild->data[leftchild->size] = tmpparent->data[childidx -
                                                                       1];//父中数据放入左孩子，父中数据肯定比左孩子都大，因此放在左孩子节点其他数据的最右侧
                    leftchild->size++;

                    for (int i = 0;
                         i < currnode->size; ++i) //currnode数据放入左孩子,currnode数据位于父的右侧，因此肯定比父中数据更大，所以依次放在左孩子节点其他数据最右侧
                    {
                ...
                if (currnode == root) {
                    ...
                    //走到这里表示是根节点，并且根节点中没数据了(原来根节点中只有一个数据，被合并后根节点中没数据了)
        ...
        void levelOrder(BTreeNode<T, M> *tNode) {
            if (tNode != nullptr) //若B树非空
            {
                ...
                _nmsp0::LinkQueue<BTreeNode<T, M> *> lnobj;//注意，队列的元素类型是“节点指针”类型
...
int main() {
    ...
    //_nmsp1::BTree<int, 5> mybtree;
    _nmsp1::BTree<int, 6> mybtree;
    for (int i = 1; i <= 30; ++i)
        mybtree.InsertElem(i);


    /*_nmsp1::BTree<int, 4> mybtree;
    mybtree.InsertElem(110);
    mybtree.InsertElem(120);
    mybtree.InsertElem(60);
    mybtree.InsertElem(50);
    mybtree.InsertElem(130);

    mybtree.InsertElem(70);
    mybtree.InsertElem(30);
    mybtree.InsertElem(40);

    mybtree.InsertElem(20);
    mybtree.InsertElem(10);
    mybtree.InsertElem(90);
    mybtree.InsertElem(80);
    mybtree.InsertElem(100);
    mybtree.InsertElem(140);
    mybtree.InsertElem(95);
    mybtree.levelOrder();

    cout << endl;
    mybtree.DeleteElem(40);
    mybtree.DeleteElem(20);
    mybtree.DeleteElem(50);
    mybtree.levelOrder();*/
    ...
    mybtree.InsertElem(70);
    mybtree.InsertElem(80);
    mybtree.InsertElem(1009);
    mybtree.InsertElem(1200);
    mybtree.levelOrder();

    cout << endl;
    mybtree.DeleteElem(10);
    mybtree.levelOrder();
    */
    mybtree.levelOrder();
    mybtree.DeleteElem(20);
