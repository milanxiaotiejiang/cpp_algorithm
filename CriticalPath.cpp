...
#define MaxVertices_size 10 //最大顶点数大小
    ...
    public:
        ...
        void DispGraph() //显示图信息
        {
            for (int i = 0; i < m_numVertices; ++i) {
                cout << i << "   入度/出度(" << m_VertexArray[i].indegree << "/" << m_VertexArray[i].outdegree << ")"
                     << " " << m_VertexArray[i].data << "：-->";   //输出顶点下标和顶点数据
                EdgeNode *ptmp = m_VertexArray[i].point;
                while (ptmp != nullptr) {
        ...
        bool TopologicalSort(int *pPopResult, int *pve) {
            ...
            {
                cout << "输出顶点数量:" << iOutputVexcount << ",而图中实际顶点数量:" << m_numVertices
                     << ",说明图中有环，没办法输出所有顶点序列（非AOV网，拓扑排序错误）" << endl;
                return false;
            }
            return true;
        }
            ...
            for (int fromVexIdx = 0; fromVexIdx < m_numVertices; ++fromVexIdx) //遍历所有顶点
            {
                ...
                while (pTmpEdge != nullptr) {
                    ...
                    if (tmpee == tmpel) {
                        ...
                        cout << "<" << m_VertexArray[fromVexIdx].data << "," << m_VertexArray[toVexIdx].data << ">(权值="
                             << toWeight << ") ";
                    }
                    pTmpEdge = pTmpEdge->next;
                }//end while
