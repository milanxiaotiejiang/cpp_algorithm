        ...
        void BlackLeaf_TuneBanance(RBNode<T> *pDelNode) //pDelNode代表要删除的节点
        {
            ...
            if (
                ...
                //(5).a 待删除节点是左节点，其父节点为黑色，兄弟节点为黑色，兄弟节点只带红色左孩子。
                    ...
                    pBroNode != nullptr && pBroNode->isRed == false &&     //兄弟黑色
                    pBroNode->rightChild == nullptr &&
                    pBroNode->leftChild != nullptr //兄弟节点只带红色左孩子(显然左孩子不可能为黑色，否则黑高不一致不是红黑树)
                    ) {
                ...
                //(6).a 待删除节点是左节点，其父节点为黑色，兄弟节点为红色且带有两个黑色的孩子(兄弟节点右孩子可以有任意红色孩子，兄弟节点左孩子不可以有孩子)。
                    ...
                    pBroNode->leftChild->leftChild == nullptr && pBroNode->leftChild->rightChild == nullptr &&
                    //且“黑色左孩子”不可以有孩子
                    pBroNode->rightChild != nullptr &&
                    pBroNode->rightChild->isRed == false  //黑色右孩子，至于是否下面还有孩子不关心，有肯定是红色，否则就不是红黑树了
                ...
                //(6).a 扩展 待删除节点是左节点，其父节点为黑色，兄弟节点为红色且带2个黑色子节点。兄弟右孩子可以有任意红色孩子，兄弟左孩子至少带一个孩子
                    ...
                    pBroNode != nullptr && pBroNode->isRed == true &&     //兄弟红色
                    pBroNode->leftChild != nullptr && pBroNode->leftChild->isRed == false &&  //黑色左孩子
                    pBroNode->rightChild != nullptr && pBroNode->rightChild->isRed == false &&
                    //黑色右孩子，至于是否下面还有孩子不关心，有肯定是红色，否则就不是红黑树了
                    !(pBroNode->leftChild->leftChild == nullptr &&
                      pBroNode->leftChild->rightChild == nullptr) //兄弟左孩子至少带一个孩子，注意前面的!
                    ) {
                RotateLeft(pParentNode); //左旋
                pParentNode->isRed = false; //新根（原兄弟）变成黑色
                ...
                //(6).b 待删除节点是右节点，其父节点为黑色，兄弟节点为红色且带有两个黑色的孩子(兄弟节点左孩子可以有任意红色孩子，兄弟节点右孩子不可以有孩子)。
                    ...
                    pBroNode->rightChild->leftChild == nullptr &&
                    pBroNode->rightChild->rightChild == nullptr //且“黑色右孩子”不可以有孩子
                    ) {
                ...
                //(6).b.扩展 待删除节点是右节点，其父节点为黑色，兄弟节点为红色且带有两个黑色的孩子，兄弟节点左孩子可以有任意红色孩子，兄弟节点右孩子至少带一个孩子
                    ...
                    pBroNode->rightChild != nullptr && pBroNode->rightChild->isRed == false &&//黑色右孩子
                    !(pBroNode->rightChild->leftChild == nullptr &&
                      pBroNode->rightChild->rightChild == nullptr) //兄弟右孩子至少带一个孩子，注意前面的!
                    ) {
                RotateRight(pParentNode); //右旋
                pParentNode->isRed = false; //新根（原兄弟）变成黑色
                ...
                assert(pParentBro->leftChild != nullptr &&
                       pParentBro->rightChild != nullptr); //断言这个节点肯定有两个孩子110，130，不然红黑树无法平衡吧
                ...
                if (pParentBro->isRed == false) //120是黑色节点
                {
                    ...
                    if (pParentNodeP->isRed == true) //100节点为红
                    {
                        ...
                        if (leftorright == 0 &&  //这个子树[以P为根]是100这个节点的左孩子
                            pParentBro->leftChild->isRed == true &&
                            pParentBro->rightChild->isRed == false) //节点120的左孩子为红色，右孩子为黑色
                        {
                            //细分2
                            ...
                            RotateLeft(pParentNodeP); //100为根左旋
                                   ...
                                   pParentBro->rightChild->isRed == true &&
                                   pParentBro->leftChild->isRed == false) //节点80的右孩子为红色，左孩子为黑
                        {
                            //细分2【反】
                ...
                else //120是红色节点，上边的100和孩子110，130都得是黑色的，否则就是红色连接到一起的。
                {
                    if (leftorright == 0) //0,这个子树是整个红黑树的左边部分
                    {
                        ...
                        if (pParentBro->leftChild != nullptr &&  //110存在
                            pParentBro->leftChild->leftChild != nullptr &&
                            pParentBro->leftChild->leftChild->isRed == false &&    //105存在且黑
                            pParentBro->leftChild->rightChild != nullptr &&
                            pParentBro->leftChild->rightChild->isRed == false)//115存在且黑
                        {
                            ...
                            //细分2
                                 ...
                                 pParentBro->leftChild->leftChild != nullptr &&
                                 pParentBro->leftChild->leftChild->isRed == true &&  //105存在且红
                                 pParentBro->leftChild->rightChild != nullptr &&
                                 pParentBro->leftChild->rightChild->isRed == false)//115存在且黑
                        {
                            //细分1，2，3的通用步骤代码，但不可提前，否则上面这些if判断条件就都要修改了
                            ...
                            //细分3
                                 ...
                                 pParentBro->leftChild->rightChild != nullptr &&
                                 pParentBro->leftChild->rightChild->isRed == true)//115存在且红
                        {
                            //细分1，2，3的通用步骤代码，但不可提前，否则上面这些if判断条件就都要修改了
                            RotateLeft(pParentNodeP); //以100为根旋转，pParentNodeP原=100，pParentNodeP新=120
                            pParentNodeP->isRed = false; //120给黑色
                    ...
                    } else  //1，这个子树是整个红黑树的右边部分
                    {
                        ...
                        if (pParentBro->rightChild != nullptr &&  //90存在
                            pParentBro->rightChild->rightChild != nullptr &&
                            pParentBro->rightChild->rightChild->isRed == false &&    //95存在且黑
                            pParentBro->rightChild->leftChild != nullptr &&
                            pParentBro->rightChild->leftChild->isRed == false)//85存在且黑
                        {
                            ...
                            //细分2【反】
                                 ...
                                 pParentBro->rightChild->rightChild != nullptr &&
                                 pParentBro->rightChild->rightChild->isRed == true &&  //95存在且红
                                 pParentBro->rightChild->leftChild != nullptr &&
                                 pParentBro->rightChild->leftChild->isRed == false)//85存在且黑
                        {
                            ...
                            //细分3【反】
                                 ...
                                 pParentBro->rightChild->leftChild != nullptr &&
                                 pParentBro->rightChild->leftChild->isRed == true)//85存在且红
                        {
                            //细分1，2，3【反】的通用步骤代码，但不可提前，否则上面这些if判断条件就都要修改了
                            RotateRight(pParentNodeP); //以100为根旋转，pParentNodeP原=100，pParentNodeP新=80
                            pParentNodeP->isRed = false; //80给黑色
                ...
                while (!lnobj.IsEmpty()) //循环判断队列是否为空
                {
                    ...
                    if (tmpvalue != -1) {
                        ...
                        if (blackheight == -1) //黑高值从来没拿到过
                        {
                            ...
                            if (blackheight != tmpvalue) {
                                ...
                                cout << "末端节点的黑高值不相等:（" << blackheight << "," << tmpvalue << "），不是红黑树了！"
                                     << endl;
                                break; //直接退出循环吧
                            }
                        }
                    ...
                    if (tmpnode->isRed == true && tmpnode->parentNd != nullptr) {
                        if (tmpnode->parentNd->isRed == true) {
                            ...
                            cout << endl; //换行
                            cout << "红黑树红色节点（" << tmpnode->data << "）的父节点（" << tmpnode->parentNd->data
                                 << "）也是红色，这已经不是一棵红黑树了！" << endl;
                            break; //直接退出循环吧
                        }
                    }
...
int main() {
    ...
    int array[] = {1212, 32, 3444, 11, 52, 3299, 5454, 8, 12, 35, 54, 3223, 3345, 4000, 6556, 2333, 3266, 3322, 3390,
                   5500, 3380, 3400, 3410};
